...
public final class MCTSRP implements MCTS {
    ...
    SearchResults<ACTION> search(int numPlayers, STATE rootState, SearchParameters params, Random rand) {
        ...
        for(int workerNum = 0; workerNum < workers.length; workerNum++) {
            ...
            workers[workerNum] = new Thread(() -> totalIters.addAndGet(rootParallelSearch(rootNode, params, rand, start)), "mctsrp" + workerNum);
            workers[workerNum].start();
        }
        totalIters.addAndGet(rootParallelSearch(rootNodes.get(0), params, rand, start));
        ...
        for(Node<STATE, ACTION> node : rootNodes) {
            final ACTION action = io.github.wallacewatler.javamcts.Node.mostVisited(node, node.validActions, rand);
