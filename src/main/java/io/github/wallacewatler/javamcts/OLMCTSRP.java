...
public final class OLMCTSRP implements OLMCTS {
    ...
    SearchResults<ACTION> search(int numPlayers, STATE rootState, SearchParameters params, Random rand) {
        ...
        final ArrayList<Node<ACTION>> rootNodes = new ArrayList<>(params.threadCount());
        ...
        for(int workerNum = 0; workerNum < workers.length; workerNum++) {
            final Node<ACTION> rootNode = rootNodes.get(workerNum + 1);
        ...
        for(Node<ACTION> node : rootNodes) {
            final ACTION action = io.github.wallacewatler.javamcts.Node.mostVisited(node, validActions, rand);
    ...
    private static
    <STATE extends VisibleState<STATE, ACTION>, ACTION extends OLMCTS.Action<STATE>>
    int rootParallelSearch(STATE rootState, Node<ACTION> rootNode, SearchParameters params, Random rand, long start) {
