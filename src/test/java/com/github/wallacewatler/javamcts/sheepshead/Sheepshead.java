...
/** Houses some of the game mechanics of Sheepshead. */
public final class Sheepshead {
    ...
    public static List<PlayCard> playableCards(List<Card> hand, Trick trickOnTable) {
        final Function<Card, PlayCard> actionMap = PlayCard::new;

        if(trickOnTable.isEmpty())
            return hand.stream().map(actionMap).toList();

        final Card ledCard = trickOnTable.ledCard();

        if(Sheepshead.isTrump(ledCard)) {
            final List<PlayCard> actions = hand.stream().filter(Sheepshead::isTrump).map(actionMap).toList();
            return actions.isEmpty() ? hand.stream().map(actionMap).toList() : actions;
        } else {
            final List<PlayCard> actions = hand.stream().filter(card -> isFail(card, ledCard.suit())).map(actionMap).toList();
            return actions.isEmpty() ? hand.stream().map(actionMap).toList() : actions;
        }
    }
