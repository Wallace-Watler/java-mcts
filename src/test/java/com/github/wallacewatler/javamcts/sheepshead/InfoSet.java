...
public final class InfoSet implements com.github.wallacewatler.javamcts.InfoSet<SheepsheadState, PlayCard> {
    public final int pov;
    public int playerAboutToMove;
    public final PlayerInfo[] playerInfos = new PlayerInfo[4];

    /** Cards that could potentially be in the blind. */
    public final ArrayList<Card> possiblyInBlind;

    /** Cards that are definitely in the blind. */
    public final ArrayList<Card> confirmedInBlind;
    public Trick trickOnTable;

    /** Create an information set from a given initial state. */
    public InfoSet(SheepsheadState initialState, int pov) {
        this.pov = pov;
        playerAboutToMove = initialState.activePlayer;
        for(int i = 0; i < 4; i++) {
    ...
    /**
     * Update this information set with the knowledge that {@code player} cannot have the cards matched by
     * {@code predicate}.
     */
    ...
    /** Update this information set with the knowledge that this card has been played. */
    ...
    @Override
    public SheepsheadState determinize(Random rand) {
        ...
        final SheepsheadState state = new SheepsheadState();
        state.activePlayer = copy.playerAboutToMove;
        for(int i = 0; i < 4; i++) {
